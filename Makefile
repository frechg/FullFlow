...
HTTPS_GIT := github.com/rollchains/fullflow.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fullflow \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/simd ./cmd/simd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/simd.exe ./cmd/simd
...
draw-deps:
	...
	@goviz -i ./cmd/simd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c fullflow --local -f chains.yaml
endif
...
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | simd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | simd keys add acc1 --recover`
